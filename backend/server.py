        ...
        try:
            async with httpx.AsyncClient(timeout=45.0) as client:
                while page_index < max_pages:
                    ...
                    data = response.json()
                    logger.info(
                        f"EPROLO page {page_index}: code={data.get('code')} msg={data.get('msg')}"
                    )

                    if not (data.get("code") == "0" or data.get("code") == 0):
            ...
            logger.info(f"EPROLO total products fetched: {len(all_products)}")
...
async def stripe_webhook(request: Request, background_tasks: BackgroundTasks):
    """Handle Stripe webhooks - verify signature, then fulfill on paid checkout"""
        ...
        if event["type"] == "checkout.session.completed":
            ...
            if session.get("payment_status") == "paid":
                ...
                if order_id:
                    ...
                    logger.info(f"Order {order_id} paid via Stripe - fulfillment triggered")
...
app.add_middleware(
    ...
    allow_origins=[o.strip() for o in os.environ.get('CORS_ORIGINS', 'https://novaxs3-frontend.onrender.com').split(',') if o.strip()],
    allow_methods=["*"],
    allow_headers=["*"],
)
